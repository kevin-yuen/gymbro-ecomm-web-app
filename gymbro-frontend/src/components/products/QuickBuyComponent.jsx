...
const QuickBuyComponent = ({ source, unitId, size }) => {
  ...
  useEffect(() => {
    size !== "NOT IN STOCK" ? setDisableQuickBuy(false) : setDisableQuickBuy(true);
  }, [])
  ...
  return (
    <div className="text-center">
      <button
        type="button"
        className={`border-0 ${!disableQuickBuy ? "custom-background-color-darkpurple" : "custom-background-color-grey"} custom-color-antiquewhite custom-font-family-inconsolata fw-bold ps-8 pe-8 pt-2 pb-2 mb-4 mt-n2 rounded-pill`}
        disabled={disableQuickBuy}
        onClick={() => handleAddToBag(source.productID, source.name, source.color, size, unitId, quickBuyQuantity)}
      >
